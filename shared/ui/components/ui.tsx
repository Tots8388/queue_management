...
import type { ConnectionState } from "@shared/ui/lib/useQueueChannel";
...
export function Button({
  ...
  // No focus-visible:outline-none here. Every button in the system inherits the
  // 3px ink ring from globals.css, and suppressing it for a softer look would
  // leave a keyboard user with nothing at all to follow.
  const base =
...
/**
 * The one text-field surface in the system.
 *
 * Defined once because it was not: the reception screen ended up rendering two
 * visibly different inputs after only half its fields were restyled. The focus
 * glow is decoration layered on top of the global ink ring — a pale tint is far
 * below the 3:1 WCAG 1.4.11 asks of a focus indicator, so it may never be the
 * only one.
 */
const FIELD_ACCENT = {
  brand: "focus:border-brand-500 focus:shadow-[0_0_0_3px_var(--color-brand-100)]",
  reception:
    "focus:border-role-reception focus:shadow-[0_0_0_3px_var(--color-role-reception-soft)]",
} as const;

export function fieldClass(
  accent: keyof typeof FIELD_ACCENT = "brand",
  className = "",
): string {
  return cx(
